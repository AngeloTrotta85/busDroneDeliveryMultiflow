...
void WorstUav::initPoi(std::map<unsigned long int, Poi> &poiMap, std::string tpsFileName) {
...
int WorstUav::updateUavPathBestPoi(Uav *uav, unsigned int t, bool reserve) {
	...
	//for (auto& pp : savingPOIs) {
	//	poisToExclude.push_back(pp);
	//}

	double worstUav = std::numeric_limits<double>::max();
	for (auto& p : backup_list_poi) {
		...
		if (!excludeThisPoi) {
			...
			double uEnergy = p->getCoveringUav()->getResudualEnergy();
	...
	if (bestPoi == nullptr) {
		worstUav = std::numeric_limits<double>::max();
		for (auto& p : backup_list_poi) {
			bool excludeThisPoi = false;
			for (auto& p2e : poisToExclude) {
				if (p->getPoiIdNum() == p2e->getPoiIdNum()) {
	...
	sim->getNeighStop(bestPoi->getPoiIdNum(), stopsEnd);
	...
	if (minCost != std::numeric_limits<int>::max()) {
		...
		for (auto itAL = newList.begin(); itAL != newList.end(); itAL++) {
...
void WorstUav::setInitExtraUAV(std::list <Uav *> &remainingUAV, struct std::tm time_tm, std::map<unsigned long int, Stops> &stopsMap) {
	...
	for (auto& uav : remainingUAV) {
		for (int j = 0; j < 50; j++) {
			cout << "Looking for the worst UAV Poi" << endl; fflush(stdout);
			int ris = updateUavPathBestPoi(uav, day_tm2seconds(time_tm), true);
			cout << "Found for the worst UAV Poi. RIS = " << ris << endl; fflush(stdout);
			if (ris == 0) {
	...
	cout << "END  setInitExtraUAV" << endl; fflush(stdout);
}

void WorstUav::activateUavFlow(unsigned int time, std::list<Uav *> &uavList){
	...
	while (!not_coveringUAV.empty()) {
		...
		if (uavArcMapList[u->getId()].size() > 0) {
			...
			for (auto itP = savingPOIs.begin(); itP != savingPOIs.end(); itP++) {
				Poi *actPoi = *itP;
				if (actPoi->getPoiIdNum() == poiDestMap[u->getId()]->getPoiIdNum()) {
					savingPOIs.erase(itP);
					break;
				}
			}
